...
const SystemModal: React.FC<{
  ...
  const initialFormData = {
    ...
    subDomain: system.subDomain || 'Ordem ServiÁo',
    platformCategory: system.platformCategory || 'Plataforma ServiÁos',
      ...
      <div className="glass-panel modal-content" style={{ maxWidth: '1100px', width: '95%' }}>
        ...
        {showCloseConfirm ? (
          <div className="confirm-delete">
            ...
            <h3>Descartar alteraÁıes?</h3>
            <p>VocÍ fez alteraÁıes neste formul·rio. Se fechar agora, perder· tudo que foi editado.</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem' }}>
              ...
              <div className="form-container">
                ...
                <div className="grid-2">
                  <div className="form-group">
                    ...
                    <select value={formData.criticality} onChange={e => setFormData({ ...formData, criticality: e.target.value as SLA })}>
                      <option value="Tier 1">Tier 1 (CrÌtico)</option>
                ...
                <div className="grid-2">
                  ...
                  <div className="form-group">
                    <label>SubdomÌnio (Categoria)</label>
                    <select 
                ...
                <div className="grid-2">
                  <div className="form-group">
                    ...
                    <select value={formData.platformCategory} onChange={e => setFormData({ ...formData, platformCategory: e.target.value })}>
                      {['Dados/IA', 'Middleware', 'Plataforma NegÛcio', 'Plataforma ServiÁos', 'Mobile', 'Portais', 'Engenharia'].map(cat => (
                ...
                <div className="form-group">
                  <label>DescriÁ„o / Finalidade</label>
                  <textarea 
              ...
              {/* Coluna 2: GovernanÁa e Detalhes TÈcnicos */}
              <div className="form-container">
                <div className="grid-2">
                  <div className="form-group">
                    <label>CustÛdia (Time)</label>
                ...
                <div className="form-group">
                  <label>Stack TecnolÛgica (separada por vÌrgula)</label>
                  <input value={formData.techStack} onChange={e => setFormData({ ...formData, techStack: e.target.value })} />
                </div>

                <div className="form-group">
                  <label>üîó RepositÛrio de CÛdigo (GitHub / Azure DevOps)</label>
                  <input 
                    type="url"
                ...
                <div style={{ borderTop: '1px solid var(--glass-border)', paddingTop: '1rem' }}>
                  <h3 style={{ fontSize: '0.85rem', marginBottom: '0.75rem', color: 'var(--accent-base)' }}>üåê Endpoints e Ambientes</h3>
                ...
                <div className="form-group">
                  <label>üìé Arquivos de Contexto</label>
                    ...
                    <div style={{ marginTop: '0.5rem', display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                      {contextFiles.map((f, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', background: 'rgba(255,255,255,0.07)', borderRadius: '4px', padding: '0.25rem 0.5rem', fontSize: '0.8rem' }}>
                          {f.name}
                          <button type="button" onClick={() => setContextFiles(prev => prev.filter((_, idx) => idx !== i))} style={{ background: 'none', border: 'none', color: 'var(--status-red)', cursor: 'pointer', padding: 0, lineHeight: 1 }}>‚úï</button>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                {canManageEntities && (
                  <div className="form-actions" style={{ marginTop: 'auto', paddingTop: '1rem' }}>
                    ...
                    <button type="submit" className="btn btn-primary" style={{ flex: 1 }}>Salvar AlteraÁıes</button>
                  </div>
                )}
              </div>
            </div>
...
const InventoryDetail: React.FC = () => {
  ...
  const handleSave = async (updated: System) => {
    ...
    try {
      ...
      alert(err.message || 'Erro ao salvar no banco de dados. A alteraÁ„o pode n„o ser persistente.');
    }
  };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <section className="glass-panel" style={{ padding: '2rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.75rem' }}>DESCRI√á√ÉO</div>
                <p style={{ fontSize: '0.95rem', lineHeight: '1.6', color: 'var(--text-secondary)', margin: 0 }}>
                  {system.description || 'Nenhuma descriÁ„o detalhada disponÌvel para este sistema.'}
                </p>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                  <Code size={13} /> STACK TECNOL√ìGICA
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
          ...
          {(system.repoUrl || (system.contextFiles && system.contextFiles.length > 0)) && (
            <section className="glass-panel" style={{ padding: '2rem' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', fontSize: '1rem' }}>
                üìÅ RepositÛrio &amp; Contexto
              </h3>
              {system.repoUrl && (
                <div style={{ marginBottom: '1.5rem' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>üîó C√ìDIGO FONTE</div>
              ...
              {system.contextFiles && system.contextFiles.length > 0 && (
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1rem' }}>üìé ARQUIVOS</div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1rem' }}>
                      {system.contextFiles.filter(f => f.type.startsWith('image/')).map((f, i) => (
                        <div key={i} style={{ borderRadius: '8px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)' }}>
                          <img loading="lazy" src={f.dataUrl} alt={f.name} style={{ width: '100%', height: '140px', objectFit: 'cover', display: 'block' }} />
        ...
        <div>
          <section className="glass-panel" style={{ padding: '1.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
              ...
              <GovernanceField icon={<Users size={13} />} label="CustÛdia (Time)">
                <div style={{ fontSize: '0.95rem', fontWeight: 600 }}>{ownerTeam?.name || 'N„o atribuÌdo'}</div>
              </GovernanceField>

              <GovernanceField icon={<User size={13} />} label="SME">
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  {sme?.photoUrl ? (
                    <img loading="lazy" src={sme.photoUrl} alt={sme.name} style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                  ) : (
                  ...
                  <div>
                    <div style={{ fontSize: '0.88rem', fontWeight: 600 }}>{sme?.name || 'N„o atribuÌdo'}</div>
              ...
              <GovernanceField icon={<Info size={13} />} label="DomÌnio">
                <div style={{ fontSize: '0.88rem', fontWeight: 600 }}>{system.domain}</div>
              </GovernanceField>

              <GovernanceField icon={<Info size={13} />} label="SubdomÌnio">
                <div style={{ fontSize: '0.88rem', fontWeight: 600 }}>{system.subDomain || '‚Äî'}</div>
              </GovernanceField>

              <GovernanceField icon={<Server size={13} />} label="Plataforma">
                <div style={{ fontSize: '0.88rem', fontWeight: 600 }}>{system.platformCategory || '‚Äî'}</div>
              </GovernanceField>
