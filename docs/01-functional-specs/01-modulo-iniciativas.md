...
- Cada iniciativa listada nas colunas é um `<a target="_blank" href="/iniciativas/:id/edit">` (tipografia/cor do texto original via `.portfolio-initiative-link` em `web/src/index.css`, sem sublinhado nem mudança de cor), mas o clique simples abre o **`InitiativePeekPanel`** (D15) em vez de navegar — o `href` só é seguido com Ctrl/⌘/Shift/clique do meio, preservando o atalho de nova aba.
...
- O painel de espiada rápida (`.peek-sidebar-container`) é o componente compartilhado `InitiativePeekPanel` (`web/src/components/initiative/InitiativePeekPanel.tsx`), renderizado via `createPortal` no `body`: `.page-content` tem `z-index: 0`, criando um stacking context que o prendia atrás das duas faixas. É usado tanto pela `InitiativesPage` (Lista/Kanban/Timeline) quanto pelo Portfólio/Roadmap do Dashboard — ver D15.
...
- Cada linha mostra a data efetiva (`dd/MM.`) e o título, com o mesmo padrão de clique do Portfólio em §3.2: abre o `InitiativePeekPanel` (D15), com o `href` para `/iniciativas/:id/edit` reservado para Ctrl/⌘/Shift/clique do meio. Quando duas ou mais linhas seguidas de uma mesma seção têm a mesma data efetiva, só a primeira exibe o texto da data — as seguintes mantêm o mesmo texto oculto por `visibility: hidden` (`.roadmap-item-date--repeated`) só para preservar a largura e manter o início do título alinhado.
...
| Data | Autor | Mudança |
|---|---|---|
| 2026-07-22 | Agente de IA (Claude) | §3.2/§3.3/§3.4 — D15: clique numa demanda do Portfólio/Roadmap abre o `InitiativePeekPanel` (extraído para `web/src/components/initiative/InitiativePeekPanel.tsx`, compartilhado com a `InitiativesPage`) em vez de navegar direto para o editor completo; Ctrl/⌘/Shift/clique do meio preservam o atalho de nova aba via `<a href>`. |
