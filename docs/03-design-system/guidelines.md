...
`ViewToolbar` aceita `showLabels` (default `false`): quando `true`, os botões icon-only viram pills com o texto da função ao lado do ícone ("Criar", "Config", "Excluir" — e, via `SearchBox`/`InitiativeFilterMenu` renderizados separadamente em `.sub-header-lead`, "Pesquisar"/"Filtro"). `SubHeader.tsx` liga `showLabels` só quando `currentView.domainFilter === 'initiative'`, então hoje só as três visões de Iniciativas mostram o texto — as demais visões com sub-header (Pessoas, Dashboard › Portfólio) continuam icon-only. Ao adicionar uma visão nova que deva usar o mesmo texto, propague por `domainFilter` (ou generalize a condição) em vez de checar o `path` diretamente.
