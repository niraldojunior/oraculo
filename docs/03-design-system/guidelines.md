...
Componentes específicos de iniciativa (editor, board, modais) vivem em `web/src/components/initiative/` e não são genéricos — não reusar fora do contexto de iniciativas sem antes avaliar se vale extrair para `common/`. Exceção: `InitiativePeekPanel.tsx` já nasceu compartilhado entre `InitiativesPage` (Lista/Kanban/Timeline) e o Dashboard (Portfólio/Roadmap) — ver D15 em [business-rules.md §12](../00-visao-geral/business-rules.md). O CSS do painel (`.peek-sidebar-container`, `.linear-sidebar-card`, `.linear-property`, `.linear-prop-label`, `.linear-prop-value`) mora em `web/src/index.css` (não mais num `<style>` de página), já que agora é consumido por duas páginas diferentes.
...
| Data | Autor | Mudança |
|---|---|---|
| 2026-07-22 | Agente de IA (Claude) | §10 — `InitiativePeekPanel` extraído da `InitiativesPage` para componente compartilhado (D15), consumido também pelo Dashboard (Portfólio/Roadmap). CSS do peek (`.peek-sidebar-container`, `.linear-sidebar-card`, `.linear-property`, `.linear-prop-label`, `.linear-prop-value`) migrado do `<style>` local da página para `web/src/index.css`; duplicata de `.linear-sidebar-card` removida do `<style>` do `InitiativeEditor`. |
