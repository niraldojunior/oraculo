...
import {
  ...
  createInitiative as createInitiativeApi,
  ...
  updateInitiativePriority as updateInitiativePriorityApi,
...
const Initiatives: React.FC = () => {
  ...
  const handleDeleteConfirmed = async () => {
    ...
    setLoading(true);
    try {
      await deleteInitiativesApi(Array.from(selectedIds));
  ...
  useEffect(() => {
    ...
    // já carregado se milestones existem com tarefas ou comentários já populados
    const alreadyHydrated = it && (it.milestones?.length || 0) > 0 && it.comments !== undefined;
    if (alreadyHydrated) return;

    Promise.all([
      fetchInitiativeById(activeInitiativeId).catch(() => null)
    ]).then(([fullInit]) => {
      setInitiatives(prev => prev.map(i =>
        i.id === activeInitiativeId
          ? {
              ...i,
              ...(fullInit || {}),
              milestones: fullInit?.milestones || i.milestones || [],
  ...
  const handleCreateSave = async (data: Partial<Initiative>) => {
    ...
    try {
      const newData = await createInitiativeApi(payload);
  ...
  React.useEffect(() => {
    ...
    .then(({ initiatives: initData, collaborators: collabsData, systems: systemsData, teams: teamsData }) => {
      const normalizedInitData: Initiative[] = initData.map(it => ({
        ...it,
  ...
  const selectedLeaderHierarchy = React.useMemo(() => {
    ...
    const allowsUnassigned = teams.some(team =>
      teamIds.has(team.id) &&
      !!team.leaderId &&
      team.leaderId !== selectedManagerId
    );

    return { teamIds, leaderIds, allowsUnassigned };
  ...
  const filteredInitiatives = React.useMemo(() => {
    return (Array.isArray(initiatives) ? initiatives : []).filter(it => {
      ...
      if (selectedManagerId !== 'all') {
        ...
        const hasManagerSignals =
          !!it.leaderId ||
          !!it.assignedManagerId ||
          (it.impactedSystemIds?.length || 0) > 0;

        if (!isLedBySelectedLeader && !isAssignedToSelectedLeader && !isImpactedBySelectedLeader) {
          // Keep unassigned initiatives visible for directors/heads that lead other leaders.
          if (!(selectedLeaderHierarchy?.allowsUnassigned && !hasManagerSignals)) {
            return false;
          }
        }
      }
  ...
  const handleUpdateInitiative = async (updated: Initiative, actionName = 'Edição rápida') => {
    ...
    try {
      const data = await updateInitiativeApi(updated.id, {
        ...payload,
  ...
  const handlePriorityUpdate = async (id: string, priority: number) => {
    try {
      const updated = await updateInitiativePriorityApi(id, priority);
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
                      ...
                      <span className="mobile-leader-avatar" title={manager?.name || 'Não atribuído'}>
                        <Avatar
                          ...
                          src={manager?.photoUrl}
                          size={24}
                          title={manager?.name || 'Não atribuído'}
                          fontSize={10}
                        />
                      </span>
                    </div>
    ...
    <div className="page-layout" style={{ 
      ...
      {milestoneDeleteId && activeInitiativeId && (() => {
        ...
        const confirmDeleteMilestone = async () => {
          ...
          try {
            await deleteMilestoneApi(currentInit.id, milestoneDeleteId);
            setInitiatives(prev => prev.map(it =>
              it.id === currentInit.id
                ? { ...it, milestones: (it.milestones || []).filter(m => m.id !== milestoneDeleteId) }
                : it
            ));
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    setInitiatives(prev => prev.map(it =>
                      it.id === initiativeId
                        ? { ...it, comments: (it.comments || []).filter(item => item.id !== comment.id) }
                        : it
                    ));
              ...
              <div className="linear-sidebar-card">
                ...
                {sidebarOpenSections.milestones && (
                  ...
                  handleUpdateMilestoneName={async () => {
                    ...
                    setInitiatives(prev => prev.map(it =>
                      it.id === initiative.id
                        ? { ...it, milestones: (it.milestones || []).map(m => m.id === editingMilestoneId ? { ...m, name: newName } : m) }
                        : it
                    ));
                    setEditingMilestoneId(null);
                    try {
                  ...
                  handleAddMilestone={async (e) => {
                    if (e.key === 'Enter' && newMilestoneName.trim()) {
                      ...
                      try {
                        ...
                        setInitiatives(prev => prev.map(it =>
                          it.id === initiative.id
                            ? { ...it, milestones: [...(it.milestones || []), milestoneForUi] }
                            : it
                        ));
                        setNewMilestoneName('');
                      } catch (err) {
                          ...
                          <button 
                            onClick={async () => {
                              ...
                              try {
                                ...
                                setInitiatives(prev => prev.map(it =>
                                  it.id === initiative.id
                                    ? { ...it, comments: [savedComment, ...(it.comments || [])] }
                                    : it
                                ));
                                setCommentText('');
                                setIsAddingComment(false);
                                        ...
                                        try {
                                          ...
                                          setInitiatives(prev => prev.map(it =>
                                            it.id === initiative.id
                                              ? {
                                                  ...it,
                                                  comments: (it.comments || []).map(item => item.id === c.id ? updatedComment : item)
                                                }
                                              : it
                                          ));
                                          setEditingCommentId(null);
