...
import { saveInitiativeWithHistory } from '../services/saveInitiative';
import {
...
const Initiatives: React.FC = () => {
  ...
  const { clientTeams: clientAreaTeams, businessUnits: clientAreaUnits } = useClientAreas();
  ...
  const peekRef = useRef<InitiativePeekPanelHandle>(null);
  ...
  const handleColumnDrop = React.useCallback(async (columnId: string) => {
    ...
    const saved = await saveInitiativeWithHistory(original, updatedInitiative, actionLabel, (user as any)?.name || 'Usuário');
    if (saved) setInitiatives(prev => prev.map(it => it.id === saved.id ? saved : it));
    ...
    setDraggedInitiativeId(null);
    setDragOverColumnId(null);
  }, [canDragBetweenColumns, draggedInitiativeId, getBoardDropPayload, initiatives, viewMode, user]);
      ...
      {activeInitiativeId && (() => {
        ...
        return (
          <InitiativePeekPanel
            ...
            systems={systems}
            onChange={updated => setInitiatives(prev => prev.map(i => i.id === updated.id ? updated : i))}
            onClose={() => setActiveInitiativeId(null)}
          />
        );
      })()}
