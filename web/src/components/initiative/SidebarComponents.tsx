...
export const renderAvatar = (collaboratorId: string | null | undefined, allCollaborators: Collaborator[], size: number = 20) => {
  ...
  return <Avatar name={collaborator.name} src={collaborator.photoUrl} size={size} title={collaborator.name} />;
};
...
export const InitiativeProperties: React.FC<SidebarSectionProps & { 
  ...
  const statusValue = formData.status || '1- Backlog';
  const typeLabel = typeValue.split('- ')[1] || typeValue;
  const statusLabel = statusValue.split('- ')[1] || statusValue;
      ...
      <div style={{ display: 'flex', alignItems: 'center', minHeight: '1.9rem' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
          {editingField === 'type' ? (
            ...
            >
              {getTypeIcon(typeValue, 16)} <span style={{ whiteSpace: 'nowrap' }}>{typeLabel}</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
          {editingField === 'status' ? (
            ...
            <div 
              ...
              style={{ fontWeight: 500, display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', color: '#1E293B', fontSize: '0.75rem' }}
            >
              <StatusIcon status={statusValue} size={16} /> <span style={{ whiteSpace: 'nowrap' }}>{statusLabel}</span>
            </div>
          )}
        </div>
      </div>
