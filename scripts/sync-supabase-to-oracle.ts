...
async function copyTable(pg: PgClient, conn: oracledb.Connection, item: TablePlan): Promise<void> {
  ...
  const source = await pg.query(`SELECT * FROM "${table}"${orderByClause(item.pk)}`);
  const rows = source.rows;

  if (rows.length === 0) {
    console.log(`[sync] ${table}: source is empty`);
    return;
  }
  ...
  console.log(`[sync] ${table}: ${rows.length} rows inserted`);
}
...
async function main() {
  ...
  try {
    ...
    console.log('[sync] Connected to Supabase (source) and Oracle (target)');

    for (const item of reverseDeleteOrder) {
      await clearOracleTable(oracle, item.table);
      console.log(`[sync] cleared Oracle table ${item.table}`);
    }
    await oracle.commit();
    ...
    console.log('\n[sync] Verification (source -> target):');
    let mismatch = 0;
    for (const item of plan) {
    ...
    if (mismatch > 0) {
      throw new Error(`[sync] Finished with ${mismatch} count mismatches.`);
    }

    console.log('\n[sync] Supabase -> Oracle synchronization completed successfully.');
  } finally {
    await pg.end();
    await oracle.close();
  }
