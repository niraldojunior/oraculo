...
import {
  ...
  saveSkill,
  saveTeam
} from '../modules/organization/services/organizationApi';
                    ...
                    {formData.memberIds.map(mid => {
                      ...
                      return (
                        <div key={mid} style={{ 
                          ...
                          <Avatar name={collab.name} src={collab.photoUrl} size={18} fontSize={9} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '0.3rem' }}>
            <div style={{ position: 'relative', marginBottom: '0.45rem' }}>
              <Avatar
                ...
                size={130}
                fontSize={44}
                style={{ border: '3px solid white', boxShadow: 'var(--shadow-lg)' }}
                backgroundColor="#E2E8F0"
                textColor="#1E293B"
              />
            </div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', maxHeight: '64px', overflowY: 'auto', padding: '0.25rem 0' }}>
                    {skill.collaborators?.map(sc => (
                      <div key={sc.collaborator.id} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: '#F8FAFC', padding: '0.2rem 0.5rem', borderRadius: '20px', border: '1px solid #E2E8F0', whiteSpace: 'nowrap' }}>
                        <Avatar name={sc.collaborator.name} src={sc.collaborator.photoUrl} size={18} fontSize={9} />
          ...
          <div ref={leftColRef} style={{ flex: 1, minHeight: 0, overflowY: 'hidden' }}>
            {filteredCollabs.map(c => (
              <div key={c.id} style={{ height: '44px', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', gap: '10px', padding: '0 15px', background: 'white' }}>
                <Avatar name={c.name} src={c.photoUrl} size={24} fontSize={10} backgroundColor="#E2E8F0" textColor="#1E293B" />
                 <div style={{ overflow: 'hidden' }}>
...
const Organization: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await fetchOrganizationPageData({
  ...
  const handleSaveAbsence = async (updated: any) => {
    try {
      ...
      const saved = await saveAbsence(updated);
  ...
  const handleSaveHoliday = async (updated: any) => {
    try {
      const isNew = !updated.id;
      const saved = await saveHoliday(updated);
  ...
  const handleSaveCollab = async (updated: Collaborator) => {
    try {
      ...
      const savedCollab = await saveCollaborator(updated);
  ...
  const handleSaveSkill = async (updated: any) => {
    try {
      const isNew = !updated.id;
      const savedSkill = await saveSkill(updated);
      setEditingSkill(null);
  ...
  const handleIncludeMembers = async (teamId: string, memberIds: string[]) => {
    try {
      const refreshed = await includeCollaboratorsInTeam(teamId, collaborators, memberIds);
  ...
  const handleRemoveMember = async (memberId: string) => {    try {
      ...
      if (!collab) return;
      await removeCollaboratorFromTeam(collab);
