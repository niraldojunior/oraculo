...
| **D13** | **Menu de 5 itens e visões endereçáveis por rota** — o menu lateral tem Dashboard, Pessoas, Produtos, Iniciativas e Tarefas (o rótulo "Pessoas" cobre as rotas `/rede/*`, mantidas por compatibilidade); cada visão dentro de uma seção tem rota própria (`/rede/hierarquia`, `/produtos/servicos/contratos`, `/iniciativas/kanban`, …), trocada por um **menu suspenso** (`ViewMenu`), nunca por *segmented control*. `ViewContext` mantém a API `activeView`/`setActiveView` mas deriva a visão do `location.pathname` — sem `localStorage`. Para adicionar uma visão, edite `navigation.ts` + `App.tsx`; não há condicional por pathname espalhada. A área `/admin` fica **fora do menu** (acesso só por URL direta), mantendo o guard `ProtectedRoute adminOnly`. | `web/src/config/navigation.ts`, `web/src/context/ViewContext.tsx`, `web/src/components/common/{ViewMenu,LeaderFilter}.tsx` |
| **D14** | **Cabeçalho em duas faixas — escopo vs. visão** — o header principal (`.top-header`) fica com o que é amplo (`ViewMenu`, filtros da seção, título/contagem, e o seletor de gestor *icon-only* ancorado no canto direito via `.header-right`); o sub-header (`.sub-header`) fica com o que é da visão atual (ícone + rótulo e as ações criar/buscar/configurar/excluir). É **opt-in por visão** via `toolbarPlacement: 'subheader'` em `ViewDef`; sem a flag, `SubHeader` retorna `null` e nada muda. As ações vêm do componente compartilhado `ViewToolbar`, usado pelas duas faixas. Páginas sem `ViewDef` (Dashboard) participam injetando `subHeaderContent`/`subHeaderTitle` pelo `ViewContext`. Hoje usam a faixa: Iniciativas (Lista, Kanban, Timeline) e a visão Portfólio do Dashboard. | `web/src/config/navigation.ts`, `web/src/components/layout/{SubHeader,Header,HeaderControls}.tsx` |
...
| **D15** | **Toda visão que lista iniciativas usa o `InitiativePeekPanel` para edição rápida**, em vez de linkar direto para o editor completo. O painel é um componente compartilhado (estado de UI, hidratação sob demanda e persistência via `saveInitiativeWithHistory` vivem nele); o host só passa `initiative`/`collaborators`/`systems` e recebe `onChange`/`onClose`. Usado hoje por Iniciativas (Lista/Kanban/Timeline) e pelo Dashboard (Portfólio/Roadmap) — o `<a href>` para `/iniciativas/:id/edit` continua existindo só para Ctrl/⌘/Shift/clique do meio. | `web/src/components/initiative/InitiativePeekPanel.tsx`, `web/src/modules/initiatives/services/saveInitiative.ts` |
...
**Navegação (D13):** para adicionar/alterar uma visão de página, edite `web/src/config/navigation.ts` (registro de seções, rotas, ícones, filtro de líder e flags de toolbar) e a rota correspondente em `web/src/App.tsx`. `Sidebar`, `Header`, `SubHeader` e `ViewMenu` derivam tudo do registro — **não** adicione condicional por `location.pathname` no `Header`, e **não** use *segmented control* (pílula) para trocar de visão. O trigger de troca de visão é **icon-only em todas as páginas** (`.view-menu-trigger--icon-only`) — sem rótulo, sem chevron, nome em `title`/`aria-label`, ícone em `--text-primary` (cinza lê como desabilitado). O Dashboard reusa as mesmas classes por ter menu próprio.

**Cabeçalho em duas faixas (D14):** as ações de visão (criar, buscar, configurar, excluir selecionados) podem ir para o sub-header em vez do header principal — basta marcar a visão com `toolbarPlacement: 'subheader'` em `navigation.ts`. As duas faixas renderizam o mesmo `ViewToolbar` (`web/src/components/layout/HeaderControls.tsx`); não duplique a lógica dos botões nem crie um sub-header próprio por página.
...
- ❌ Não duplique os botões de toolbar (add/search/settings/delete) numa página ou num header próprio — use `ViewToolbar` e a flag `toolbarPlacement` (D14).
